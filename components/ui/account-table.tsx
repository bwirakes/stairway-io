...
interface AccountType {
  id: number; // Changed from number to string since it's category name
        ...
        <div className="sm:flex sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-gray-700 dark:text-gray-400">
              A list of all your {title.toLowerCase()} including account details and values.
            </p>
          </div>
              ...
              {filteredData.map((account) => (
                <React.Fragment key={account.id}>
                  <TableRow className="hover:bg-gray-50 dark:hover:bg-neutral-800">
                    ...
                    <TableCell
                      className={`text-right ${account.isAsset ? 'text-green-600' : 'text-red-600'}`}
                    >
                      {account.value}
                    </TableCell>
                    <TableCell>
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => toggleRow(account.id.toString())}
                        className="p-0"
                      >
                        {expandedRows.has(account.id.toString()) ? (
